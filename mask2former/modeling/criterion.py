...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
                 num_points, oversample_ratio, importance_sample_ratio, ood_loss, margin):
    ...
    def loss_ood(self, outputs, targets, indices, num_masks):
        ...
        if self.ood_loss == "margin":
            ...
            score = F.interpolate(score.unsqueeze(1), size=ood_masks_.shape[-2:], mode="bilinear", align_corners=True).squeeze(1)
            ...
            if ood_mask.sum() > 0:
                loss = loss + F.binary_cross_entropy_with_logits(ood_score, torch.ones(ood_score.size()).cuda(), reduction="mean")
            loss = 0.5 * loss
        else:
