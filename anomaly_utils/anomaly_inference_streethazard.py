...
if __name__ == "__main__":
    ...
    if args.input:
        for path in glob.glob(os.path.expanduser(str(args.input[0]))):
            ...
            if args.output:
                ...
                outputs_na = 1 - torch.max(predictions_naa[0, 0:12,:,:].unsqueeze(0), axis = 1)[0]
                outputs_na = outputs_na.detach().cpu().numpy().squeeze().squeeze()

                #left-right
                predictions_lrr =  predictions_lr["sem_seg"].unsqueeze(0)
                outputs_lr = 1 - torch.max(predictions_lrr[0, 0:12,:,:].unsqueeze(0), axis = 1)[0]
                outputs_lr = outputs_lr.detach().cpu().numpy().squeeze().squeeze()
                outputs_lr = np.flip(outputs_lr.squeeze(), 1)
                

                outputs = np.expand_dims((outputs_na + outputs_lr)/2.0, 0).astype(np.float32)

                
                pathGT = path.replace("images", "labels")                

                mask = Image.open(pathGT)
                ood_gts = np.array(mask)
          
                if "streethazard" in pathGT:
                    ood_gts = np.where((ood_gts==1), 255, ood_gts)
                    ood_gts = np.where((ood_gts==4), 255, ood_gts)
                    ood_gts = np.where((ood_gts<14), 0, ood_gts)
    ...
    file.write(str('   AUPRC score:' + str(prc_auc) + '   FPR@TPR95:' + str(fpr) ))
    file.close()
